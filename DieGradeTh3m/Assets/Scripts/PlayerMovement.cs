...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void CounterMovement(float x, float y, Vector2 mag)
    {
        ...
        if (crouching)
        {
            rb.AddForce(orientation.transform.forward * slideForce / 2);
        ...
        if (isDashing)
        {
            if (rb.velocity.magnitude >= maxSpeed)
            {
                rb.AddForce(-orientation.transform.forward * dashSpeed / 2);
            }
            return;
        }
    ...
    private void Dashing()
    {
        ...
        {
            rb.AddForce(orientation.right * dashSpeed, ForceMode.Impulse);
        }
        ...
        else rb.AddForce(orientation.forward * dashSpeed, ForceMode.Impulse);
